...
import org.purecelibacy.sadhana.activities.habits.edit.*
import org.purecelibacy.sadhana.activities.habits.list.views.*
import org.purecelibacy.sadhana.activities.intro.LoginFragment
...
import org.purecelibacy.sadhana.core.ui.callbacks.*
import org.purecelibacy.sadhana.core.ui.screens.habits.list.*
import org.purecelibacy.sadhana.core.ui.screens.habits.list.ListHabitsBehavior.Message.*
...
@Inject constructor(
        ...
        private val monthPickerFactory: MonthPickerFactory,
        private val behavior: Lazy<ListHabitsBehavior>,
        private val menu: Lazy<ListHabitsMenu>,
        private val selectionMenu: Lazy<ListHabitsSelectionMenu>,
        private val remarkPickerFactory: RemarkPickerFactory
) : BaseScreen(activity),
    ...
    override fun showProfileScreen() {
        LoginFragment.setFirstTime(false);
        val intent = intentFactory.startProfileActivity(activity)
        activity.startActivity(intent)
    }
...
/*        val dialog = AlertDialog.Builder(activity)
        ...
        val builder = AlertDialog.Builder(activity,R.style.YourDialogStyle)
        builder.setTitle(Html.fromHtml("<font color='#000000'>Type of Sadhana</font>"))
        builder.setItems(R.array.habitTypes) { _, which ->
            if (which == 0) showCreateBooleanHabitScreen()
            else showCreateNumericalHabitScreen()
        }
    ...
    fun showImportScreen() {
        ...
        //intent.type = "*/*";
        //activity.startActivityForResult(intent, REQUEST_OPEN_DOCUMENT)
       // activity.startActivityForResult(intent, Intent.ACTION_GET_CONTENT)
    }
    ...
    override fun showMessage(m: ListHabitsBehavior.Message) {
        showMessage(when (m) {
                        ...
                        EXPORT_SUCCESSFUL -> R.string.export_successfull
    ...
    override fun showNumberPicker(value: Double,
        ...
        numberPickerFactory.create(value, unit,remark,habit,callback).show()
    }

    override fun showRemarkPicker(remark: String?,
                                  habit: Habit,
                                  callback: ListHabitsBehavior.RemarkPickerCallback) {
        remarkPickerFactory.create(remark,habit,callback).show()
    }

    override fun showMonthPicker(sadhanaFileName: String,
                         unit: String,
                         callback: ListHabitsBehavior.MonthPickerCallback) {
        monthPickerFactory.create(sadhanaFileName, unit, callback).show()
    }
    ...
    private fun onExportDB() {
        taskRunner.execute(exportDBFactory.create { filename ->
            if (filename != null) {
                ...
                showLongMessage(R.string.export_data_successfull)
                //showSendFileScreen(filename)
            }
