...
class HabitCardView(
    ...
    var remakrsCheckmark
        get() = checkmarkPanel.remarks
        set(remakrsCheckmark) {
            checkmarkPanel.remarks = remakrsCheckmark
            // numberPanel.values = remakrs.map { it / 1000.0 }.toDoubleArray()
        }

    var remakrs
        get() = numberPanel.remarks
        set(remakrs) {
            numberPanel.remarks = remakrs
            // numberPanel.values = remakrs.map { it / 1000.0 }.toDoubleArray()
        }
    ...
    init {
        ...
        checkmarkPanel = checkmarkPanelFactory.create().apply {
            onToggle = { timestamp,withRemark ->
                triggerRipple(timestamp)
                habit?.let { behavior.onToggle(it, timestamp,withRemark) }
            }
            onEdit = { timestamp ->
                triggerRipple(timestamp)
                habit?.let { behavior.onEdit(it, timestamp,true) }
            }
        }
        ...
        numberPanel = numberPanelFactory.create().apply {
            ...
            onEdit = { timestamp ->
                triggerRipple(timestamp)
                habit?.let { behavior.onEdit(it, timestamp,false) }
            }
        }
