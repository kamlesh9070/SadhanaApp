...
package org.youth.sadhana.core.models.memory;
...
public class MemoryRepetitionList extends RepetitionList
{
    ...
    @Override
    public int getCountByInterval(Timestamp fromTimestamp, Timestamp toTimestamp)
    {
        int count = 0;
        ArrayList<Repetition> filtered = new ArrayList<>();
        ...
        for (Repetition r : list)
        {
            ...
            if(habit.isNumerical())
                if(r.getValue() < habit.getTargetValue()*1000) continue;
            if (t.isOlderThan(fromTimestamp) || t.isNewerThan(toTimestamp)) continue;
            count++;
        }
