...
*/
    ...
    /*public String writeMonthlyReport() throws IOException {
        writeMultipleHabits();
        return exportDirName + generateFilenames.get(1);
    }*/
    ...
    private void writeMultipleHabits() throws IOException
    {
        ...
        /*if(fromDate != null && toDate != null) {
            Date date = fromDate.toJavaDate();
            SimpleDateFormat sdf = new SimpleDateFormat("MMM");
            checksFileName = fileName + "_" + sdf.format(date) + ".csv";
        }*/
        ...
        //writeMonthNameHeader(checksWriter);
        ...
        for (int i = 0; i <= days; i++)
        {
            ...
            for(int j = 0; j < selectedHabits.size(); j++)
            {
                ...
                /*if(selectedHabits.get(j).isNumerical()) {
                    if(checkmarks.get(j) [days-i] >= 1000) {
                        checkmarks.get(j) [days-i] = checkmarks.get(j) [days-i]/1000;
                    }
                    checksWriter.write(String.valueOf(checkmarks.get(j) [days-i] ));
                }
                else {
                    if(checkmarks.get(j) [days-i]  > 0) {
                        checksWriter.write("Y");
                    } else {
                        checksWriter.write("N");
                    }
                }*/

                checksWriter.write(DELIMITER);
                String score = String.format("%.4f", ((float) scores.get(j)[i]));
                        //String.format("%.4f", ((float) scores.get(j) [days-i] ));
  ...
  /*  private void writeMonthNameHeader(Writer out) throws IOException
    {
...
*/
       /* Date date = fromDate.toJavaDate();
        try {
            out.write("Month:" + DELIMITER);
            SimpleDateFormat sdf = new SimpleDateFormat("MMM yyyy");
            out.write(sdf.format(date) + DELIMITER);
        } catch (Exception e) {
            e.printStackTrace();
        }
        out.write("Center:" + DELIMITER);
        out.write(preferences.getMBACenter()+DELIMITER);
        out.write("Name:"+DELIMITER);
        out.write(preferences.getFirstName()+ " " + preferences.getLastName() + DELIMITER);
        out.write("\n");
    }  */
    ...
    private Timestamp[] getTimeframe()
    {
        ...
        //Timestamp newest = Timestamp.ZERO.plus(1000000);
        //Timestamp oldest = Timestamp.ZERO;
        for (Habit h : selectedHabits)
        {
