...
package org.purecelibacy.sadhana.core.models;
...
import static org.purecelibacy.sadhana.core.models.Checkmark.*;
import static org.purecelibacy.sadhana.core.utils.StringUtils.defaultToStringStyle;
...
public abstract class CheckmarkList
{
                                                        ...
                                                        ArrayList<Interval> intervals)
    {
        ...
        for (Repetition rep : reps)
        {
            ...
            int offset = date.daysUntil(today);
            checkmarks.set(offset, new Checkmark(date, CHECKED_EXPLICITLY,remark));
        }
    ...
    public abstract Map<String,Integer> getGroupByMonth();
    public abstract List<Checkmark> groupBy(DateUtils.TruncateField field);
    public abstract List<Checkmark> toList();
    ...
    public final String[] getRemkars(Timestamp from, Timestamp to)
    {
        if (from.isNewerThan(to)) return new String[0];

        List<Checkmark> checkmarks = getByInterval(from, to);
        String values[] = new String[checkmarks.size()];

        int i = 0;
        for (Checkmark c : checkmarks)
            values[i++] = c.getRemark();

        return values;
    }


    public final List<Checkmark> getCheckmarks(Timestamp from, Timestamp to) {
        if (from.isNewerThan(to)) return null;
    ...
    private void computeNumerical(Repetition[] reps)
    {
        ...
        for (Repetition rep : reps)
        {
            ...
            checkmarks.set(offset, new Checkmark(rep.getTimestamp(), rep.getValue(), rep.getRemark()));
        }
