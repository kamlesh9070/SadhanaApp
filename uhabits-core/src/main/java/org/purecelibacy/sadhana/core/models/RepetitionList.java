...
public abstract class RepetitionList
{
    ...
    public abstract int getCountByInterval(Timestamp fromTimestamp, Timestamp toTimestamp);
    /**
    ...
    public synchronized Repetition toggle(Timestamp timestamp)
    {
        ...
        {
            if(rep != null)
                rep = new Repetition(timestamp, Checkmark.CHECKED_EXPLICITLY,rep.getRemark());
            else
    ...
    @NonNull
    public synchronized Repetition toggle(Timestamp timestamp, String remark)
    {
        if(habit.isNumerical())
            throw new IllegalStateException("habit must NOT be numerical");
        ...
        {
            rep = new Repetition(timestamp, Checkmark.CHECKED_EXPLICITLY,remark);
            add(rep);
        }
    ...
    public void toggle(Timestamp timestamp, int value)
    {
        ...
        if(rep != null) remove(rep);
        if(rep != null)
            add(new Repetition(timestamp, value,rep.getRemark()));
        else
